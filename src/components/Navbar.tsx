...
import {AiOutlineClear as ClearIcon, AiOutlineCopy as CopyIcon, AiFillLayout as LayoutICon, AiFillEdit as EditICon} from 'react-icons/ai'
  ...
  return (
    <Menu>
      ...
      <MenuList>
        {
          menuItems.map(item => (
            <MenuItem key={item.value} onClick={item.method}>{item.value}</MenuItem>
          ))
        }
      </MenuList>
    </Menu>
...
const WYSIWYGControls = ({setMarkdown}: ControlsProps) => {
  return (
    <Box display="flex" flexDir={{base: 'column', lg: 'row'}} alignItems='flex-end' flexWrap={{base: 'wrap-reverse', lg: 'nowrap'}} h={{base: '300px', lg: 'auto'}} gap="10px" color="#fff">
...
const DesktopNavbar = ({children}: DesktopNavbarProps) => {
  return (
    <Box display={{base: 'none', lg: 'flex'}} alignItems='center' justifyContent='space-around' w='100%' h='100%'>
      {children}
    </Box>
  )
}
...
const MobileNavbar = ({children}: DesktopNavbarProps) => {
  return (
    <Box display={{base: 'flex', lg: 'none'}} alignItems='center' justifyContent='space-between' w='100%' h='100%' px='10px'>
      {children}
    </Box>
  )
}
...
const Navbar = () => {
  ...
  return (
    <Box as="nav" w="100%" h="45px" backgroundColor="#282c34" borderBottom="1px solid rgb(255, 255, 255, 0.3)">
      <DesktopNavbar>
        ...
        <Box display='flex' gap='10px'>
          ...
          <CustomMenu
            name='Layout menu'
            icon={<LayoutICon />}
            menuItems={[{value: 'Horizontal', method: () => setOrientation('horizontal')}, {value: 'Vertical', method: () => setOrientation('vertical')}]}
          />
          <CustomMenu
            name='File options'
            icon='File'
            menuItems={[{value: 'Copy Markdown', method: () => handleCopy(md.code)}, {value: 'Copy HTML', method: () => handleCopy(html.code)}]}
          />
        </Box>
        <Link href='/'>
          <Button leftIcon={<BsArrowLeftShort />} size='sm' variant='outline'>Home</Button>
        </Link>
      </DesktopNavbar>
      <MobileNavbar>
        <Box display='flex' gap='10px'>
        ...
        <Box zIndex={10} pos='absolute' bottom='10px' right='10px' display='flex' flexDir='column' alignItems='flex-end' gap='10px'>
          {
            showEditTools
            ? <WYSIWYGControls setMarkdown={setMarkdown}/>
            : undefined
          }
          <IconButton aria-label='toggle' icon={<EditICon />} size='sm' variant='outline' onClick={() => setShowEditTools(show => !show)}/>
        </Box>
      </MobileNavbar>
    </Box>
  )
